...
HOST, PORT = '127.0.0.1', 45558
description = 'I am welcome server. I listening {}-{}. I send string Welcome to the client.'.format(HOST, PORT)
...
def connection(name, sock):
    try:
        while True:
            ...
            if data == 'Hello':
                print(name, data)
                sock.sendall('Welcome to our local network'.encode('utf-8'))
            else:
...
if __name__ == '__main__':
    ...
    welcomeServer = threading.Thread(target=serverSocket)
    welcomeServer.start()
    nameServer.start()
